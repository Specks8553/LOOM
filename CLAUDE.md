...
**Any feature work whose Hard Blocker (HB-\*) is unticked is a bug. Stop, resolve the blocker, then continue.** Soft Blockers (SB-_) are required for substrate (Phase 0); Cross-Doc inconsistencies (CD-_) and Schema/IPC drift (SD-_, IP-_) must be resolved before the touching feature ships.
