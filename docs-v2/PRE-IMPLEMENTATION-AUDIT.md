...
**2026-05-14 — Phase 5 implementation verification (Source Documents).** Phase 5 surface (HB-2, CD-8, SD-1, SD-2, IP-4) shipped under verified contracts; no new audit findings.

Per-item notes:

- **HB-2** — `attachment_history` schema honored in [src-tauri/src/db/migrations/world/001_initial.sql](src-tauri/src/db/migrations/world/001_initial.sql): columns `event` + `reason TEXT NULL`. Phase 5 writes verified by cargo tests `db::attachment_history::tests::*` (attach NULL reason; detach with `reason='soft_delete'`; detach with NULL reason) and `services::vault::tests::soft_delete_cascades_detach_with_reason` (cascade writes the documented reason on every affected story).
- **CD-8 / IP-4** — `attach_context_doc` / `detach_context_doc` return `Result<Vec<String>, LoomError>` in [src-tauri/src/commands/vault.rs](src-tauri/src/commands/vault.rs); typed wrappers in [src/lib/tauriApi/vault.ts](src/lib/tauriApi/vault.ts) return `Promise<string[]>`. Frontend (`workspaceStore.attachDoc` / `.detachDoc`) consumes the returned ordering and reloads `attachedDocs` once via `listAttachedDocs`; no extra round-trip for the array order itself, per the doc.
- **SD-1 / SD-2** — Reconciled at audit time (HB-2). Phase 5 implementation observes both: `event` column written via [src-tauri/src/db/attachment_history.rs](src-tauri/src/db/attachment_history.rs); `reason` column written as `Some("soft_delete")` only by `services::vault::cascade_detach_on_soft_delete`, NULL on user detaches.
