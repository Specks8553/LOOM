...
- **2026-05-16: Phase 9 implemented (frontend-only).** The backend `update_feedback` command + `[WRITER FEEDBACK]` history injection already shipped in Phase 3; Phase 9 is purely the affordance.
  - `globals.css` — added the `--color-feedback[-hover/-subtle]` triad (⚠️ provisional, NB-1; independent of `--color-warning` per Doc 28).
  - `workspaceStore` — `feedbackEditingMessageId: string | null` field + `beginFeedbackEdit` / `cancelFeedbackEdit` / `commitFeedbackEdit` actions. `commitFeedbackEdit` reuses the existing `updateFeedback` IPC wrapper then clears the flag. Reset on story switch / clear; `enterGhostwriter` also clears it (Doc 28 §With Ghostwriter).
  - `FeedbackStrip.tsx` — new component: renders nothing when feedback empty + editor closed; single-line truncated preview when non-empty; inline editor (textarea + hint + Cancel/Apply) when this bubble's edit is open. Textarea value is local component state; only the editing *fact* is global. Esc cancels (`stopPropagation` so no other escape slot fires); Ctrl/Cmd+Enter applies. Cached-message guard on Apply via `useCachedMessageGuard`.
...
- **Decisions:** (a) the 6 colour keys are full `AppSettingKey` variants (App + World), reconciling Doc 03's "world-only" annotation with Doc 20 §Features "App + World" and `ResolvedSettings`'s cascade — Doc 03 amended 2026-05-16. (b) Templates are world-scoped (the `templates` table lives only in `loom.db`); the "App scope" templates in Doc 20 §Templates have no store — treating all templates as per-world. (c) `export_settings_bundle` + Startup-behaviour radio deferred from the General tab (need backup infra / a new schema key — outside the 5 checkpoints). (d) Developer-tab diagnostic toggles (Verbose logging, API Debug Preview) omitted — no backend wiring exists; defer to a later pass.
- **2026-05-16: Frontend landed.** `lib/theme.ts` (`applyTheme` + tone derivation), `lib/settingsSchema.ts` (validators mirror Rust), `lib/tauriApi/settings.ts`, `stores/settingsStore.ts`, `appStore.settingsOpen`. `components/settings/`: `Settings` (chapter switcher, tab list, search, ⚑ filter, Escape), `SettingField` (6 control kinds, debounced save, cascade ↺), `PromptEditor`, `TemplatesTab`, `SettingsTabContent` (7 App + 5 World tabs). Wired into `WorkspaceShell` content switching (Settings > DocEditor > Story) + Navigator; `applyTheme` effect at App root (Doc 20 triggers 1–3; trigger 4 in store actions).
- **Automated gates green:** cargo build/clippy (`--all-targets`) clean; 227 Rust tests pass; tsc + eslint clean; 37 vitest tests pass.
- **Pending:** the 5 Testable Checkpoints need manual verification in a running Tauri build (`pnpm tauri dev`) — run `/phase-verify`. Implementation otherwise complete.
