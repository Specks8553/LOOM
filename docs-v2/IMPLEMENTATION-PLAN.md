...
- **2026-05-15: 7B landed (history substitution).** Uncommitted on `main`.
  - `services/history.rs::build_history_with_accordion(branch_messages, segments, checkpoints, fake_user_prompt) -> Vec<GeminiContent>` — pure function. Walks chronological messages, locates the active segment via `(start_anchor, end_anchor]` windows derived from checkpoint anchor `created_at` (start sentinel anchor = ""). Substitutes a single fake-pair (`fake_user_prompt` → `summary`) once per qualifying segment (`summary.is_some() && (is_collapsed || use_summary)`); underlying messages in that segment are dropped from output. Fast path returns literal rendering when `segments.is_empty()`.
  - `services/history.rs::assemble_story_request` + `assemble_session_request` — both now load `(segments, checkpoints)` via `db::accordion::list_*` and call the new helper instead of the per-message render loop. New required field `fake_user_prompt: &'a str` on `AssembleInputs` and `SessionAssembleInputs`.
  - `services/cache.rs::build_story_prefix` — same swap: replaced the per-message loop with `build_history_with_accordion`. Added `segments`, `checkpoints`, `fake_user_prompt` to `StoryPrefixInputs`. The cached high-water message id (`last_cached_message_id`) is still the chronologically-last *underlying* message (not the fake-pair), so cached-prefix overlap checks against the cache's story messages line up with `messages` rows on disk.
  - `services/cache.rs::build_cache_prefix` (story arm) — loads accordion state + resolves `PromptAccordionFakeUser` from the cascade, threads through `StoryPrefixInputs`.
  - All four story callers in `commands/conversation.rs` (send_message inline, send retry, get_token_count) and the session caller in `commands/modes.rs::send_session_message` now resolve `PromptAccordionFakeUser` and pass it through. Token-count helper picks up post-substitution body for free.
  - **Consulting cache rebuild (`build_session_prefix` in cache.rs) intentionally untouched** — Doc 22 mandates that re-entry uses the snapshot's accordion state (not the current state). Snapshot capture is plumbed via Phase 6C + 7A. Implementing snapshot-based substitution at rebuild is a refinement (the snapshot has summaries but not per-segment message id ranges) and is out of scope for 7B per the original phase plan. Inline consulting sends (cache disabled) DO get current-state substitution via the shared `assemble_session_request` path.
  - 6 new tests in `services/history.rs::tests`: literal pass-through with no segments, single-segment full substitution, segment-with-no-summary pass-through, segment with `!is_collapsed && !use_summary` pass-through (writer reading raw), open-segment messages pass-through, and a DB-backed integration test exercising `assemble_story_request` end-to-end. `cargo test --lib`: 192 passed (+6). `cargo clippy --lib --all-targets -- -D warnings` clean.
  - **Next session (7C — summarisation + stale):** implement `services/accordion.rs::summarise_segment` (non-streaming Gemini call using `gen_summarise_*` + `prompt_accordion_summarise` SI, `isGenerating` gate, rate-limit check, cancellation via global token); add cache-stale wiring to the accordion mutation commands (`mark_story_stale` when segment range overlaps cached prefix); extend `conversation.rs::{update_message_content, edit_user_message, regenerate_last_response, delete_exchange, delete_from, update_feedback}` to call `services/accordion::mark_segment_stale_for_message` and emit `accordion_state_changed`.

- **2026-05-15: 7C landed (summarisation + stale).** Uncommitted on `main` alongside the in-flight 7C cache helper.
  - `services/gemini.rs::generate_content` (new) — non-streaming `:generateContent` POST. Mirrors `stream_generate` for cancellation semantics: token-fire before send or before body parse returns `GenerateOutcome { cancelled: true, .. }` with no error. Two wiremock tests cover the success and cancel paths.
  - `services/accordion.rs::build_summarise_request` (new) — pure DB → `AssembledRequest` builder. Loads the segment's `(start_anchor, end_anchor]` story-kind messages, runs them through `history::render_message_into` (now `pub(crate)`), errors with `LoomError::validation` on empty range. `store_summarise_result` is a thin wrapper around `update_segment_summary` so the command imports a single name.
  - `services/cache.rs::segment_overlaps_cached_prefix` (already uncommitted from in-flight 7C prep) — true when the segment's start anchor sits strictly before the cache high-water message's `created_at`. The supporting `segment_start_anchor_created_at` helper treats the start sentinel as `""`.
  - `commands/accordion.rs::summarise_segment` (new) — async Tauri command. Resolves `TextModelName`, `PromptAccordionSummarise`, and `GenSummarise{Temperature,TopP,TopK,MaxOutputTokens}` via the cascade; calls `build_summarise_request`; installs a fresh cancellation token via `access::install_cancel_token` (replaces any previous); awaits `gemini::generate_content`. Cancellation returns `Ok(None)` silently per Doc 16 §Cancellation. On success: `store_summarise_result`, `maybe_mark_cache_stale_for_segment`, emit `accordion_state_changed`. Returns `Option<String>` (`None` on cancellation). Registered in `lib.rs::invoke_handler`.
  - `commands/accordion.rs` — cache-stale wiring added to every mutation: `update_segment_summary`, `clear_segment_summary`, `set_segment_use_summary`, `summarise_segment` use `maybe_mark_cache_stale_for_segment` (gated on `cache::segment_overlaps_cached_prefix`). `create_checkpoint` and `delete_checkpoint` use `maybe_mark_cache_stale_for_story` (any active cache → stale; per Doc 22 split/merge the rare non-overlapping case is acceptable to widen-invalidate rather than diff segment ranges). `rename_checkpoint` and `set_segment_collapsed` remain non-stale-triggers per Doc 16.
  - `commands/conversation.rs::mark_segment_stale_for_message` (new helper) — called immediately after `mark_story_cache_stale_for_message` in every edit/regenerate/delete/feedback path: `update_message_content`, `edit_user_message`, `regenerate_last_response`, `delete_exchange`, `delete_from`, `update_feedback`. The pre-delete ordering is preserved (helper runs BEFORE `db_delete_exchange` / `db_delete_from` / truncate / regen-pop so the message is still resolvable). Looks up `messages.story_id` from the row, calls `services::accordion::mark_segment_stale_for_message`, emits `accordion_state_changed` only when a segment was actually marked (open-segment messages yield no-op).
  - `services/history.rs::render_message_into` — visibility lifted from `fn` to `pub(crate) fn` so `services/accordion.rs::build_summarise_request` can reuse the canonical user/model rendering (parses `json_user`, appends feedback to model turns).
  - Two new tests in `services::accordion::tests`: `build_summarise_request_renders_segment_messages` (verifies user + model both rendered in order, no `cached_content_name`, SI threaded through) and `build_summarise_request_errors_on_empty_segment` (deletes the only message in a segment then asserts `LoomError::Validation`). Two new tests in `services::gemini::tests`: `generate_content_returns_concatenated_text_and_metadata` and `generate_content_returns_cancelled_when_token_already_fired`.
  - **Rate limiter — intentionally deferred.** Doc 16 §Pre-flight requires a `text`-provider rate-limit check before summarise. No rate-limiter module exists yet (no `services/rate_limiter.rs`); the same gap applies to existing story sends. Phase 7C does NOT introduce one — gating is handled client-side via the frontend `isGenerating` flag (Phase 7D will wire the button-greyed UI). Adding the rate-limiter module is its own future phase.
  - **isGenerating — server-side gate not added.** v2 keeps the `isGenerating` gate client-side (CLAUDE.md §6, Doc 15). The cancellation-token install on each call already ensures only one in-flight summarise/send (a new install drops the previous token clone, so cancelling the new token has no effect on the old worker — but the worker still completes; UI prevents starting a second). Matches `send_message` / `regenerate_last_response` precedent.
  - `cargo build`, `cargo test --lib` (196 passed, +4 new), `cargo clippy --lib --all-targets -- -D warnings` (clean), `tsc --noEmit` (clean).
  - **Checkpoints still unticked** — every Phase 7 Testable Checkpoint exercises the UI ("Collapse a 5-exchange segment, summarise it → next request…", etc.). Those land at the end of 7D. The backend pieces are in place: invoking `summarise_segment` from the frontend will produce a summary, write it, and the next send will auto-substitute via 7B's `build_history_with_accordion`.
  - **Next session (7D — frontend):** workspaceStore — `checkpoints[]`, `segments[]`, 8 actions (`getAccordionState`, `createCheckpoint`, `renameCheckpoint`, `deleteCheckpoint`, `updateSegmentSummary`, `setSegmentCollapsed`, `setSegmentUseSummary`, `clearSegmentSummary`) + `summariseSegment` async action; `accordion_state_changed` listener that re-fetches state. `AccordionBanner` component (extends Phase 4's partition banner with token-impact display, ⚠ stale badge, "Generate summary" button, chevron collapse toggle). Theater renders banners at their checkpoint positions between messages; bubbles hidden inside collapsed segments. Right-click "Insert checkpoint here" on AI bubbles (overlay menu pattern). Right-click menu on banner: rename / delete / summarise / re-summarise / clear summary / toggle use_summary. Cached-message guard wraps `createCheckpoint`/`deleteCheckpoint` when the affected range overlaps cache. vitest for store reducers + the listener's `getAccordionState` round-trip.

- **2026-05-15: 7D landed (frontend).** Uncommitted on `main`.
  - `src/lib/tauriApi/accordion.ts` (new) — 9 typed wrappers covering every accordion command: `getAccordionState`, `createCheckpoint`, `renameCheckpoint`, `deleteCheckpoint`, `updateSegmentSummary`, `setSegmentCollapsed`, `setSegmentUseSummary`, `clearSegmentSummary`, `summariseSegment`.
  - `src/stores/workspaceStore.ts` — slice extended: `checkpoints: Checkpoint[]`, `segments: AccordionSegment[]`, `summarisingSegmentIds: Set<string>`. 8 actions mirror the IPC names + a 9th `loadAccordionState` for the listener path. `setActiveStory` now pulls accordion state alongside messages / draft / attached docs (Promise.all). `clear()` drops the slice. Per-segment spinner via `summarisingSegmentIds` so a banner can show progress without raising the global `isGenerating` (the backend cancellation-token install already serialises generations across the app).
  - `src/hooks/useWorkspaceEvents.ts` — registers `accordion_state_changed`. Listener filters to the active story id and re-fetches via `loadAccordionState` (full-state refresh; surgical re-fetch with the event's `segment_id?` / `checkpoint_id?` is an optimisation worth leaving until 7D ships and the cost is observed).
  - `src/components/theater/AccordionBanner.tsx` (new) — implements the Doc 16 §Banners spec: chevron toggle (disabled until a summary exists; collapse forbidden without one), button-slot state machine (open segment → none, no summary → "Generate summary", summarising → spinner, summary + expanded → "Use summary" toggle, summary + collapsed → toggle hidden), `⚠` stale badge, inline-rename textbox, inline summary editor, summary card body when collapsed. Right-click menu (overlay pattern, no Dialog primitive yet per Doc 09 deferral) with all Doc 16 §Banner right-click menu actions including "Summarise previous chapter" / "Re-summarise previous chapter" wired to `previousSegment`.
  - `src/components/theater/TheaterBody.tsx` — `buildRenderItems` extended to interleave accordion banners. Sort key `__a__` (messages), `__b__` (session partitions), `__c__` (banners) at the same anchor time keeps the start sentinel at the top (anchor "" + special key `__a__sentinel`) and user banners immediately after the anchor AI bubble. Messages whose containing segment is `is_collapsed = true` are filtered out of the inline stream (banner shows the summary card in their place per Doc 16 §Banner state matrix row 1). Per-segment message count is computed from `(startAt, endAt]` ranges over story-kind messages.
  - `src/components/theater/StoryAIBubble.tsx` — right-click → `Insert checkpoint here` overlay item. Suggests `"Chapter N"` (N = existing user checkpoints + 2). Disabled while `isGenerating`.
  - `src/__tests__/workspaceStoreAccordion.test.ts` (new) — 5 vitest specs: load round-trip, `createCheckpoint` invokes with `{storyId, afterMessageId, name}` + refetches, `summariseSegment` flips the per-segment spinner on/off on resolve and on rejection, `clear()` drops accordion slice. Mocks `@tauri-apps/api/core` per the project's existing pattern.
  - **Cached-message guard not extended to checkpoint create/delete.** The existing `useCachedMessageGuard` covers message edits/deletes per Doc 22 §Cached-message Edit/Delete Protection. The backend already auto-stales the cache when a checkpoint touches the cached prefix (Phase 7C `maybe_mark_cache_stale_for_story`). Adding a frontend confirmation modal for checkpoint-time invalidation is a UX polish — deferred to a future pass. Existing `window.confirm` on delete preserves "this cannot be undone" parity.
  - **Server-side `isGenerating` parity.** `summariseSegment` does not raise the global `isGenerating` flag in the store. Reason: doing so would also disable Send / Edit / Regenerate buttons app-wide, which a writer summarising an old chapter probably doesn't want. The backend's cancellation-token install still serialises actual generations — issuing a Send during a summarise will replace the token and the summarise will finish (its result is persisted) but its cache-stale emit will be cosmetically delayed. If this turns into a real footgun the per-segment spinner approach can be tightened to a global gate in a polish pass.
  - **Verification:** `cargo build` clean, `cargo test --lib` 196 passed, `cargo clippy --lib --all-targets -- -D warnings` clean, `tsc --noEmit` clean, `eslint src --max-warnings 0` clean, `vitest run` 31 passed (+5 new).
  - **Manual UI checkpoints still pending.** Phase 7's Testable Checkpoints ("Collapse a 5-exchange segment, summarise it → next request sends fake-pair…" etc.) require a running Tauri shell with an unlocked vault and a valid Gemini key. The full wiring is in place: invoking `summariseSegment` from the banner writes the summary; the next send's `assemble_story_request` calls `build_history_with_accordion` which substitutes the fake-pair (covered by 7B's tests). Tick after `/phase-verify` walks them manually.
